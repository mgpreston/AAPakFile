...
public class PackageCompactor
{
        ...
        {
            {
                ...
                await using var editor = await PackageEditor
                    .CreateAsync(tempPath, xlGamesKey, cancellationToken)
                    .ConfigureAwait(false);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await WriteFileTableAsync(handle, writeOffset, updatedRecords, state.Header,
            encryptor, cancellationToken).ConfigureAwait(false);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var stream = new FileStream(handle, FileAccess.Write, bufferSize: 80 * 1024, isAsync: true);
        ...
        var dif = stream.Position % 512;
        if (dif > 0)
            await stream.WriteAsync(new byte[512 - dif], cancellationToken).ConfigureAwait(false);
        ...
        await stream.WriteAsync(new byte[headerBlockSize - encryptedHeaderSize], cancellationToken).ConfigureAwait(false);
